...
type GraphData = {
  ...
  links: Array<{ source: string | number; target: string | number; [k: string]: any }>;
};
...
export default function CoalNetworkGraph({ dataUrl, height, freeLayout = true }: Props) {
  ...
  const [selected, setSelected] = useState<{ node: any; x: number; y: number } | null>(null);
  const [query, setQuery] = useState("");
  const fgRef = useRef<any>(null);
  const fitOnceRef = useRef<boolean>(false);
  const [groups, setGroups] = useState<Array<{ id: number; label?: string; description?: string; meta?: Record<string, any>; nodes: any[] }>>([]);
  ...
  useEffect(() => {
    ...
    fetch(dataUrl)
      ...
      .then((json) => {
        ...
        // ensure free layout
        g.nodes.forEach((n: any) => { delete n.x; delete n.y; delete n.vx; delete n.vy; });
        setData(g);
        ...
        fetch('/coal/coalGroup_info.json')
          ...
          .then((info) => {
            ...
            g.nodes.forEach((n: any) => { byId[n.id] = n; });
            const comps: Array<{ id: number; label?: string; description?: string; meta?: Record<string, any>; nodes: any[] }> = [];
            Object.keys(info).forEach((k) => {
              ...
              (gi.nodes || []).forEach((nid: any) => {
                const n = byId[nid];
                if (n) { n.groupId = gid; nodes.push(n); }
              });
              const { nodes: _omit, ...rest } = gi;
              comps.push({ id: gid, label: gi['Retirement archetype'], description: gi['Drivers for planned retirements'], meta: rest, nodes });
            });
            // Assign ungrouped by connectivity
            const ungrouped = (g.nodes as any[]).filter(n => typeof n.groupId !== 'number');
            if (ungrouped.length) {
              let next = comps.length ? Math.max(...comps.map(c => c.id)) + 1 : 0;
              ...
              for (const n of ungrouped) {
                ...
                while (stack.length) {
                  ...
                  cur.groupId = next;
                  bucket.push(cur);
                  (cur.neighbors || []).forEach((nbr: any) => { if (!seen.has(nbr)) { seen.add(nbr); stack.push(nbr); } });
                }
  ...
  const matchSet = useMemo(() => {
    ...
    data.nodes.forEach((n: any) => {
      ...
      for (const p of plants) {
        ...
        if (name.includes(q)) { s.add(n); break; }
      }
    });
    return s;
  }, [data, query]);
  ...
  const componentSet = useMemo(() => {
    ...
    if (activeGroup != null) {
      ...
      if (grp) { s.clear(); grp.nodes.forEach((n)=>s.add(n)); }
    }
    return s;
  ...
  useEffect(() => {
    ...
    try {
      ...
      setTimeout(() => { try { fgRef.current.zoom(k * 0.9, 300); } catch {} }, 350);
    } catch {}
  }, [query]);
  ...
  const safeEntries = (obj: any) => {
    ...
    const omit = new Set(["neighbors", "links", "x", "y", "vx", "vy", "index", "__indexColor", "plants"]);
      ...
      <div ref={wrapRef} className="relative flex-1" style={{ height: "100%" }}>
        ...
        <div className="pointer-events-auto absolute left-3 top-3 z-10 flex w-[min(520px,90%)] items-center gap-2">
          <input
            ...
            className="w-full rounded-md border border-slate-300 bg-white/90 px-3 py-2 text-sm outline-none placeholder:text-slate-400 backdrop-blur focus:border-slate-500 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-100"
          />
          <button
            type="button"
            ...
            onClick={() => { try { fgRef.current?.zoomToFit(400, 40); } catch {} }}
          >
            Reset View
          </button>
        </div>
      ...
      {data && typeof ForceGraph2D !== "undefined" && (
        <ForceGraph2D
          ref={(el: any) => {
            ...
            if (el && typeof el.zoomToFit === 'function' && !fitOnceRef.current) {
              fitOnceRef.current = true;
              setTimeout(() => {
                try { el.zoomToFit(400, 40); } catch {}
              }, 120);
            }
          }}
          width={width}
          ...
          linkDirectionalParticleWidth={(lk: any) => (highlightLinks.has(lk) ? 4 : 0)}
          nodeCanvasObjectMode={(n: any) => (highlightNodes.has(n) || matchSet.has(n) ? 'before' : undefined)}
          nodeCanvasObject={paintRing}
          linkColor={(lk: any) => {
            if (activeGroup == null) return (highlightLinks.has(lk) ? 'rgba(59,130,246,0.9)' : 'rgba(148,163,184,0.5)');
            const src = (lk.source as any);
            const tgt = (lk.target as any);
            const inGrp = (src?.groupId === activeGroup) && (tgt?.groupId === activeGroup);
            return inGrp ? (highlightLinks.has(lk) ? 'rgba(59,130,246,0.9)' : 'rgba(148,163,184,0.7)') : 'rgba(148,163,184,0.2)';
          }}
        ...
        {groups.length > 0 && (
          <div className="pointer-events-auto absolute right-3 top-3 z-10 flex items-center gap-2 rounded-md bg-white/85 p-2 text-sm shadow dark:bg-slate-900/80">
            ...
            <select
              ...
              value={activeGroup ?? ''}
              onChange={(e) => setActiveGroup(e.target.value === '' ? null : Number(e.target.value))}
            >
              <option value="">All</option>
              {groups.map((g) => (
                <option key={g.id} value={g.id}>{g.id}{g.label ? ` – ${g.label}` : ''} ({g.nodes.length})</option>
              ))}
            </select>
            <button
        ...
        {selected?.node && (
          <aside className="pointer-events-auto absolute right-0 top-0 z-10 flex h-full w-[380px] flex-col border-l border-slate-200 bg-white/90 p-3 backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/80">
            <div className="mb-3 flex items-center justify-between">
              <div className="text-sm text-slate-600 dark:text-slate-300">Node ID: <span className="font-medium text-slate-900 dark:text-slate-100">{String(selected.node.id)}</span></div>
              <button
            ...
            {typeof (selected.node as any).groupId === 'number' && (
              <div className="mb-3 rounded-md border border-slate-200 p-3 text-xs dark:border-slate-700">
                {(() => {
                  ...
                  return (
                    <div>
                      <div className="mb-2 font-medium text-slate-800 dark:text-slate-100">Group {gid}{grp?.label ? `: ${grp.label}` : ''}{grp?.meta?.Name ? ` – ${grp.meta.Name}` : ''}</div>
                      {grp && (
                        <dl className="mb-2 grid grid-cols-[160px_1fr] gap-x-3 gap-y-1 text-slate-700 dark:text-slate-300">
                          {grp.label && (
                            <>
                              <dt className="text-slate-500 dark:text-slate-400">Retirement archetype</dt>
                              <dd>{grp.label}</dd>
                            </>
                          )}
                          {grp.description && (
                            <>
                              <dt className="text-slate-500 dark:text-slate-400">Retirement drivers</dt>
                      ...
                      >Snap to Group</button>
                    </div>
                  );
                })()}
              </div>
            )}
            <div className="flex min-h-0 flex-1 flex-col rounded-md border border-slate-200 dark:border-slate-700">
              <div className="border-b border-slate-200 bg-slate-50 px-3 py-2 text-xs font-medium uppercase tracking-wide text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
                Plants ({Array.isArray((selected.node as any).plants) ? (selected.node as any).plants.length : 0})
              </div>
              <div className="min-h-0 flex-1 overflow-auto">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {Array.isArray((selected.node as any).plants) && (selected.node as any).plants.map((p: any, i: number) => (
                      <tr key={(p["Plant Name"] ?? "") + String(i)} className="border-t border-slate-100 dark:border-slate-800">
                        <td className="px-3 py-2 text-slate-900 dark:text-slate-100">{p["Plant Name"] ?? "–"}</td>
                        <td className="px-3 py-2 text-slate-700 dark:text-slate-300">{p["Retirement Date"] ?? "N/A"}</td>
                        <td className="px-3 py-2 text-slate-700 dark:text-slate-300">{p["Utility Name"] ?? "–"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
